...
class TabKANet(nn.Module):
    def __init__(self, 
                 vocabulary: Dict[str, Dict[str, int]], num_continuous_features: int,
                 embedding_dim: int, nhead: int, num_layers: int, dim_feedforward: int, attn_dropout_rate: float,
                 layers_hidden: List[int], grid_size=5 , spline_order=3 , scale_noise=0.1, scale_base=1.0, scale_spline=1.0,
        ...
        self.classifier = KAN( layers_hidden, grid_size=5, spline_order=3, scale_noise=0.1, scale_base=1.0, scale_spline=1.0, base_activation=torch.nn.SiLU, grid_eps=0.02, grid_range=[-1, 1],)
